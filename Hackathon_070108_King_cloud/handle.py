...
closlight_lis = [u"把灯关闭", u"关灯", u"把灯关掉", u"关闭灯"]
homeweather_list = [u"家里什么情况", u"现在家里什么情况"]
video_list = [u"宝贝在干嘛", u"视频监控", u"在干嘛"]
outdorweather_list= [u"今天天气怎么样", u"今天的天气"]
music_rand_list = [u"我要听音乐", u"来一首音乐", u"听音乐", u"播放音乐"]
music_jay_list = [u"周杰伦歌曲", u"周杰伦的歌曲", u"来首周杰伦的歌曲"]
music_andy_list = [u"刘德华歌曲", u"来首刘德华歌曲"]
music_stop_list = [u"停止吧", u"停止音乐", u"停止", u"停了吧"]
map_list = [u"宝贝在哪呢", u"在哪呢", u"定位宝贝"]
# key和url填入自己申请到的图灵key以及图灵请求url
tl_auto_reply = auto_reply.TulingAutoReply("", "http://www.tuling123.com/openapi/api")
class Handle(object):
    def GET(self):
        try:
            data = web.input()
        ...
        try:
            ...
            if isinstance(recMsg, receive.Msg):
                ...
                if recMsg.MsgType == 'text':
                    ...
                    video_val = compare.compare_to_list(unicode_str, video_list)
                    if cmp1_val is True:
                        print cmp1_val
                    ...
                    print voice_content
                     #如果满足是用户自定义的单词就选择图灵机器人
                    cmp1_val = compare.compare_to_list(voice_content, openlight_list)
                    cmp2_val = compare.compare_to_list(voice_content, closlight_lis)
                    cmp3_val = compare.compare_to_list(voice_content, homeweather_list)
                    cmp4_val = compare.compare_to_list(voice_content, outdorweather_list)
                    cmp5_val = compare.compare_to_list(voice_content, map_list)
                    rand_val = compare.compare_to_list(voice_content, music_rand_list)
                    jay_val = compare.compare_to_list(voice_content, music_jay_list)
                    andy_val = compare.compare_to_list(voice_content, music_andy_list)
                    stop_val = compare.compare_to_list(voice_content, music_stop_list)
                    video_val = compare.compare_to_list(voice_content, video_list)
                    if cmp1_val is True:
                        print cmp1_val
                        pub.pub_msg("{\"ctr_light\":1}", "cmd")
                        tl_reply = u"好的，打开灯"
