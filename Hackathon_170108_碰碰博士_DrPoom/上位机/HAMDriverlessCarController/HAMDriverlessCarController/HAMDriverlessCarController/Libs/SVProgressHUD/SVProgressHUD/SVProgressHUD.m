...
+ (void)resetOffsetFromCenter {
    [self setOffsetFromCenter:UIOffsetMake(0, 0)];
}
...
- (id)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        ...
        SVProgressHUDBackgroundColor = [SVProgressHUD colorWithHex:0x000000 alpha:0.7f];
...
- (NSTimeInterval)displayDurationForString:(NSString*)string {
    return MIN((float)string.length*0.12 + 0.5, 5.0);
}
