...
def getPhotosFromDirectory(photoDir):
    return [f for f in os.listdir(photoDir) if
            os.path.isfile(os.path.join(photoDir, f)) and os.path.splitext(f)[1].lower() == ".jpg"]
...
if sys.platform == "win32":
    ...
    bundlerExecutable = os.path.join(distrPath, "software/bundler/bin/bundler")
...
class OsmBundler():
    ...
    def __init__(self):
        ...
        self.workDir = 'F:\workspace\osm-bundler'
    ...
    def preparePhotos(self, *kargs, **kwargs):
        ...
        if ('featureExtractionNeeded' in kwargs and kwargs[
            'featureExtractionNeeded'] == False) or self.matchingEngine.featureExtractionNeeded == False:
    ...
    def checkCameraInDatabase(self, photoPath):
        ...
        if 'Make' in exif and 'Model' in exif:
            ...
            if ccdWidth == None:
                while True:
                    print "Type CCD width in mm for the camera %s, %s. Press Enter to skip the camera." % (
                    exifMake, exifModel)
                    ...
                    try:
                        ...
                        self.dbCursor.execute("INSERT INTO cameras(make, model, ccd_width, source) VALUES(?, ?, ?, 2)",
                                              (exifMake, exifModel, ccdWidth))
                        ...
                        print "CCD width %s for the cameras %s,%s has been successively inserted to the database" % (
                        ccdWidth, exifMake, exifModel)
                        return
    ...
    def _preparePhoto(self, photoInfo):
        ...
        scale = 0
        if self.photoScalingFactor:
            scale = self.photoScalingFactor
        else:
        ...
        if 'Make' in exif and 'Model' in exif:
            ...
            if ccdWidth:
                if 'FocalLength' in exif and 'ExifImageWidth' in exif and 'ExifImageHeight' in exif:
                    ...
                    width = float(exif['ExifImageWidth'][0])
                    height = float(exif['ExifImageHeight'][0])
                ...
                logging.info("\tEntry for the camera '%s', '%s' does not exist in the camera database" % (
                exif['Make'], exif['Model']))
        if not hasFocal:
            logging.info(
                "\tCan't estimate focal length in pixels for the photo '%s'" % os.path.join(photoInfo['dirname'],
                                                                                            photoInfo['basename']))
    ...
    def doBundleAdjustment(self):
        ...
        subprocess.call([bundlerExecutable, "list.txt", "--options_file", "options.txt"],
                        **dict(stdout=bundlerOutputFile))
